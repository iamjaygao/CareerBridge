...
const SuperAdminTopbar: React.FC<SuperAdminTopbarProps> = ({ onMenuClick }) => {
  const navigate = useNavigate();
  const dispatch = useDispatch();
  const { user } = useSelector((state: RootState) => state.auth);
  const { setOverrideRole, isSuperAdmin } = useRole();
    ...
    >
      <Toolbar sx={{ minHeight: '64px !important', px: { xs: 2, md: 3 } }}>
        ...
        >
          ...
          <InputBase
            placeholder="Search users, mentors, appointments, system..."
            value={searchQuery}
