...
from .services import enrich_mentor_value_copy, build_mentor_contract
...
class HumanReviewTaskSerializer(serializers.ModelSerializer):
    """Serializer for Human Review Task model"""
    ...
    class Meta:
        ...
        fields = (
            'id', 'signal', 'decision_slot_id', 'task_type', 'status', 'priority',
            'assigned_to', 'assigned_to_user', 'assigned_at', 'review_notes',
            'review_decision', 'reviewed_at', 'context_data', 'due_at',
            'is_critical', 'is_overdue', 'created_at', 'updated_at'
        )
        read_only_fields = ('id', 'created_at', 'updated_at', 'assigned_at', 'reviewed_at')
    
    def get_signal(self, obj):
        """Get signal details"""
        from ats_signals.serializers import ATSSignalSerializer
        if obj.signal:
            return ATSSignalSerializer(obj.signal).data
        return None
