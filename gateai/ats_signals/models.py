...
class ATSSignal(models.Model):
    """
    ATS Signal model for storing engine-generated signals.
    
    Stores signals generated by engines (e.g., ResumeAuditEngine), anchored to DecisionSlot ID.
    This is the OS-native signal storage for the ats_signals layer.
    
    Rule 15: SIGNAL INTEGRITY - Every signal MUST be anchored to a DecisionSlot ID.
    """
    # Signal Integrity (Rule 15)
    decision_slot_id = models.CharField(max_length=100, db_index=True, help_text="DecisionSlot ID anchoring this signal")
    
    # Signal metadata
    signal_type = models.CharField(
        max_length=50,
        help_text="Type of signal (structure_issue, content_issue, keyword_missing, etc.)"
    )
    severity = models.CharField(
        max_length=20,
        help_text="Severity level (critical, high, medium, low, info)"
    )
    category = models.CharField(max_length=200, help_text="Category of the issue/strength")
    ...
    section = models.CharField(max_length=100, null=True, blank=True, help_text="Resume section where issue/strength was found")
    line_number = models.IntegerField(null=True, blank=True, help_text="Line number if applicable")
    
    # Engine metadata
    engine_name = models.CharField(max_length=100, help_text="Name of the engine that generated this signal")
    engine_version = models.CharField(max_length=50, help_text="Version of the engine")
    signal_schema_version = models.CharField(max_length=20, default='1.0.0', help_text="Version of the signal schema")
    ...
    class Meta:
        ...
        indexes = [
            ...
            models.Index(fields=['signal_type', 'severity']),
    ...
    def __str__(self):
        return f"Signal {self.id} - {self.signal_type} ({self.decision_slot_id})"
    
    @property
    def is_critical(self):
        """Check if signal is critical severity."""
        return self.severity == 'critical'
    
    @property
    def is_high_priority(self):
        """Check if signal is high priority (critical or high severity)."""
